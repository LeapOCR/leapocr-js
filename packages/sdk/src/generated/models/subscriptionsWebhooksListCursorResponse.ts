/**
 * Generated by orval v7.16.0 🍺
 * Do not edit manually.
 * LeapOCR API
 * Advanced OCR API for processing PDF documents with AI-powered text extraction using Gemini LLM integration. Supports structured data extraction, template-based processing, and real-time job management.
 * OpenAPI spec version: v1
 */
import type { SubscriptionsWebhookSubscriptionResponse } from './subscriptionsWebhookSubscriptionResponse';

export interface SubscriptionsWebhooksListCursorResponse {
  data?: SubscriptionsWebhookSubscriptionResponse[];
  has_more?: boolean;
  next_cursor?: string;
  total_count?: number;
  total_pages?: number;
}
